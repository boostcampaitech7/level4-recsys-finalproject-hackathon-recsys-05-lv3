...
def bert_pca(input_csv_path, batch_size=8, pca_components=128, model_name="bert-base-uncased"):
    """
    주어진 CSV 파일의 'overview' 컬럼에 대해 BERT 임베딩 생성 후,PCA를 적용하여 차원을 축소한 임베딩을 반환
    
    Parameters:
        input_csv_path (str): 입력 CSV 파일 경로. 'overview' 컬럼을 포함해야 함
...
def word2vec(input_csv_path, vector_size=100, window=5, min_count=1, workers=4):
    """
    주어진 CSV 파일의 'overview' 컬럼 텍스트에 대해 Word2Vec 임베딩 생성 후, 각 단어의 벡터를 DataFrame 형태로 반환
